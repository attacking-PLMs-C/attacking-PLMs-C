...
def convert_examples_to_features(item):
    ...
    #source
    s_code = url_to_code[url1]+url_to_code[url2]
    s_code1 = url_to_code[url1]
    ...
    return InputFeatures(s_code1, source_tokens, source_ids, position_idx, dfg_to_code, dfg_to_dfg, url1+'_'+url2,label)
...
def test(args, model, tokenizer):
    ...
    for batch in tqdm(eval_dataloader,total=len(eval_dataloader)):
        ...
        with torch.no_grad():
            ...
            total_code.extend(s_code)

    logits=np.concatenate(logits,0)
    labels=np.concatenate(labels,0)
    preds=logits[:,0]>0.5
    with open('original_cases.txt', 'w') as fp:
        for i in tqdm(range(preds.shape[0])):
            if preds[i] == labels[i]:
                # print(total_idx[i])
                id1_ = total_idx[i].split('_')[0]
                id2_ = total_idx[i].split('_')[1]
                # print(id1_.split('/')[-1])
                id1 = id1_.split('/')[1]+'_'+id1_.split('/')[-1]
                id2 = id2_.split('/')[1]+'_'+id2_.split('/')[-1]
                subprocess.check_output('cp ./dataset/code/' + id1 + '  ./initial_code/', shell=True)
                subprocess.check_output('cp ./dataset/code/' + id2 + '  ./code2/', shell=True)
                fp.write(id1+'  '+id2+'  '+str(int((preds[i])))+'  '+str(labels[i])+'\n')
